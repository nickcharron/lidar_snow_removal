...
public:
    ...
    */
    ...
    template<typename T>
    void Filter(typename pcl::PointCloud<T>::Ptr &input_cloud, typename pcl::PointCloud<T> &filtered_cloud);
...
template<typename T>
void DROR::Filter(typename pcl::PointCloud<T>::Ptr &input_cloud, typename pcl::PointCloud<T> &filtered_cloud) {
...
//    Clear points in output cloud
    ...
    for (typename pcl::PointCloud<T>::iterator it = input_cloud->begin();
        ...
        int neighbors = kd_tree_->radiusSearch(*it, search_radius_dynamic, pointIdxRadiusSearch,
